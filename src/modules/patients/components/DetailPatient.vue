  ...
  <div class="space-y-6 p-4 max-w-7xl mx-auto text-gray-800 dark:text-gray-200 relative">
    ...
    <div v-else-if="patient" class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          class="lg:col-span-2 bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm flex flex-col justify-between"
        >
          <div>
            <div class="flex items-center gap-4 mb-4">
              <div
                class="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 font-bold text-base tracking-wider"
              >
              ...
              <div>
                ...
                <span
                  class="inline-flex items-center text-xs font-mono bg-blue-50 text-blue-600 dark:bg-blue-500/10 px-2.5 py-0.5 rounded-lg font-bold mt-1"
                >
                  ID: {{ patient.code }}
                </span>
              </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-6 pt-2">
              ...
              <div>
                ...
                <span
                  class="inline-flex text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 px-2.5 py-0.5 rounded-lg border border-red-100 dark:border-red-500/20"
                >
                  {{ patient.blood_group }}
                </span>
              </div>
        ...
        <div
          class="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm flex flex-col justify-center space-y-4"
        >
          <h3 class="text-xs font-bold uppercase tracking-wider text-gray-400">Coordonnées</h3>
          <div class="flex items-center gap-3">
            <div
              class="w-8 h-8 rounded-lg bg-gray-50 dark:bg-white/[0.05] flex items-center justify-center text-gray-500 dark:text-gray-400"
            >
              <svg
            ...
            <a
              ...
              class="text-sm font-semibold hover:text-blue-600 transition-colors"
              >{{ patient.phone }}</a
            >
          </div>
          <div class="flex items-center gap-3">
            ...
            <a
              ...
              class="text-sm font-medium hover:text-blue-600 transition-colors truncate block max-w-[220px]"
              >{{ patient.email }}</a
            >
          </div>
        </div>
      ...
      <div
        class="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden"
      >
        <div
          class="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gray-50/50 dark:bg-white/[0.01]"
        >
          <div>
            <h2 class="text-lg font-bold text-gray-900 dark:text-white">
              Dossier Clinique & Historique
            </h2>
            <p class="text-xs text-gray-400 mt-0.5">
              Cliquez sur une ligne pour ouvrir les détails de la consultation
            </p>
          </div>
          <div class="relative w-full sm:w-80">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Rechercher un diagnostic, code, date..."
              class="w-full pl-9 pr-4 py-2 text-xs rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            />
          </div>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr
                class="border-b border-gray-100 dark:border-gray-800 text-[11px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50/50 dark:bg-transparent"
              >
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800/60">
              <tr v-if="filteredRecords.length === 0">
                <td colspan="6" class="text-center py-12 text-xs text-gray-400">
              ...
              >
                ...
                <td class="py-4 px-6 whitespace-nowrap">
                  <div class="flex flex-col gap-0.5">
                    <span class="text-[10px] font-mono text-gray-400"
                      >RDV: {{ record.appointment_code }}</span
                    >
                    <span
                      v-if="record.hasConsultation"
                      class="text-[10px] font-mono font-bold text-blue-600 dark:text-blue-400"
                      >CONS: #{{ record.consultation_id }}</span
                    >
                  </div>
                </td>
                ...
                <!-- Diagnostic léger -->
                <td
                  class="py-4 px-6 max-w-xs truncate font-medium text-gray-700 dark:text-gray-300"
                >
                  <span
                    v-if="!record.hasConsultation"
                    class="text-gray-300 dark:text-gray-700 italic font-normal"
                    >En attente</span
                  >
      ...
      <div class="flex items-center justify-between pt-2">
        ...
        <button
          type="button"
          @click="$router.back()"
          class="px-5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-50 dark:hover:bg-gray-800 transition-all text-xs shadow-sm"
        >
          Retour à la liste
        </button>
      </div>
      ...
      <div v-if="isDrawerOpen" class="fixed inset-0 z-50 overflow-hidden">
        ...
        >
          ...
          >
            <div>
              ...
              <p class="text-xs text-gray-400 font-mono mt-0.5">
                Code RDV: {{ selectedRecord?.appointment_code }}
              </p>
            </div>
            <button
              @click="closeDrawer"
          ...
          <div class="p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
            <div
              class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-50 dark:bg-white/[0.01] p-4 rounded-2xl border border-gray-100 dark:border-gray-800/80"
            >
              <div class="text-center sm:text-left">
                <span class="block text-[10px] uppercase font-bold text-gray-400 mb-0.5"
                  >Type d'acte</span
                >
                <span
                  :class="[
                    'inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase',
                    selectedRecord?.hasConsultation
                ...
                >
                  {{
                    selectedRecord?.hasConsultation
                      ? 'Consultation #' + selectedRecord.consultation_id
                      : 'Rendez-vous Simple'
                  }}
                </span>
              </div>
              <div class="text-center sm:border-l dark:border-gray-800 px-2">
                ...
                <span class="text-xs font-semibold dark:text-white">
                  {{
                    selectedRecord?.hasConsultation
                      ? formatDateTime(selectedRecord.consultation_date)
            ...
            <div
              v-if="selectedRecord && !selectedRecord.hasConsultation"
              class="py-12 text-center border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-2xl"
            >
            ...
            <div v-else-if="selectedRecord" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div class="space-y-6">
                <div class="space-y-2">
                  ...
                  <div
                    class="text-sm font-medium text-gray-900 dark:text-white bg-gray-50 dark:bg-white/[0.02] p-5 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm leading-relaxed min-h-[120px] whitespace-pre-line"
                  >
                ...
                <div class="space-y-2">
                  ...
                  <div
                    class="text-sm font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50/40 dark:bg-emerald-500/[0.02] p-5 rounded-2xl border border-emerald-100 dark:border-emerald-800/30 shadow-sm min-h-[80px] whitespace-pre-line"
                  >
              ...
              <div class="space-y-6">
                <div class="space-y-2">
                  ...
                  <div
                    class="text-xs font-mono text-gray-800 dark:text-gray-200 bg-blue-50/30 dark:bg-blue-500/[0.01] p-5 rounded-2xl border border-blue-100/50 dark:border-blue-800/30 shadow-sm min-h-[230px] whitespace-pre-line leading-relaxed tracking-wide"
                  >
                    <div
                      class="border-b border-blue-100/50 dark:border-blue-900/30 pb-2 mb-3 text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider text-[10px]"
                    >
              ...
              <div
                v-if="selectedRecord.notes && selectedRecord.notes !== '...'"
                class="col-span-1 lg:col-span-2 space-y-2"
              >
                <h4 class="text-xs font-bold uppercase tracking-widest text-gray-400">
                  Notes & Remarques confidentielles
                </h4>
                <div
                  class="p-4 rounded-2xl bg-gray-50/50 dark:bg-white/[0.01] border border-gray-100 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400 italic"
                >
          ...
          <div
            class="p-6 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950 flex gap-3"
          >
            <button
              @click="closeDrawer"
...
const combinedRecords = computed(() => {
  return appointments.value
    .map((app) => {
      const matchingConsultation = consultations.value.find((c) => c.appointment_code === app.code)
