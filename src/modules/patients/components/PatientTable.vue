        ...
        <table class="min-w-full table-fixed">
          <thead>
            ...
            >
              ...
              <th
                class="w-1/5 px-6 py-3.5 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase"
              >
                Signes Vitaux
              </th>
              <th
                class="w-1/6 px-6 py-3.5 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase"
              >
                Statut
              </th>
              <th
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800/60">
            ...
            <tr v-if="isLoading">
              <td colspan="5" class="px-6 py-12 text-center">
                <div class="flex flex-col items-center justify-center gap-3">
                  <div
                    class="h-6 w-6 animate-spin rounded-full border-2 border-solid border-teal-600 border-r-transparent"
                  ></div>
            ...
            >
              ...
              <td class="px-6 py-3.5 whitespace-nowrap">
                <p class="text-xs font-medium text-gray-700 dark:text-zinc-300 truncate">
              ...
              <td class="px-6 py-3.5 whitespace-nowrap">
                <div class="flex items-center gap-2.5 text-xs">
                  <div>
                    ...
                    <span
                      class="font-mono bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 px-1.5 py-0.5 rounded text-gray-700 dark:text-zinc-300"
                      >{{ patient.vitals.bp }}</span
                    >
                  </div>
              ...
              <td class="px-6 py-3.5 whitespace-nowrap">
                <span
                  :class="[
                    'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold border tracking-wider',
    ...
    <Teleport to="body">
      <Transition name="slide">
        <div v-if="selectedPatient" class="fixed inset-0 z-50 overflow-hidden">
          ...
          ></div>
          <div
            class="absolute inset-y-0 right-0 max-w-sm w-full bg-white dark:bg-zinc-950 border-l border-gray-100 dark:border-zinc-900 shadow-xl flex flex-col"
          >
            <!-- Drawer Header -->
            <div
            ...
            <div class="flex-1 overflow-y-auto bg-gray-50/30 dark:bg-zinc-950/30 custom-scrollbar">
              <div class="p-5 space-y-6">
                ...
                <div
                  class="flex flex-col items-center text-center p-4 rounded-xl bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 shadow-sm"
                >
                  <div
                    :class="[
                      ...
                      isMale(selectedPatient.gender)
                        ...
                        : 'bg-gradient-to-br from-rose-50 to-rose-100 text-rose-600 border-rose-200',
                    ]"
                  >
                    {{ isMale(selectedPatient.gender) ? '♂' : '♀' }}
                  </div>
                  ...
                  <div class="flex gap-2 mt-4">
                    <div
                      class="px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700"
                    >
                      <p class="text-[10px] uppercase font-bold text-gray-400 mb-0.5">Sexe</p>
                      <p class="text-xs font-semibold dark:text-white">
                        {{ isMale(selectedPatient.gender) ? 'Homme' : 'Femme' }}
                      </p>
                    </div>
                    <div
                      class="px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700"
                    ...
                    >
                      ...
                      <p class="text-xs font-semibold dark:text-white">
                        {{ calculateAge(selectedPatient.dob) }} ans
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div
                  v-if="selectedPatient && !selectedPatient.is_claimed"
                  class="bg-teal-50 dark:bg-teal-500/10 border border-teal-100 dark:border-teal-500/20 rounded-xl p-4 flex flex-col items-center text-center shadow-sm"
                >
                  <p
                    class="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider mb-1"
                  >
                    Code Application Mobile
                  </p>
                  <p class="text-xs text-gray-500 dark:text-zinc-400 mb-3">
                    Le patient doit saisir ce code lors de son inscription sur l'application mobile.
                  </p>
                  <div class="flex items-center gap-2">
                    <span
                      class="text-xl font-black font-mono tracking-widest text-gray-900 dark:text-white px-4 py-1.5 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg shadow-sm"
                    >
                      {{ selectedPatient.id }}
                    </span>
                  </div>
                ...
                <div
                  v-else-if="selectedPatient && selectedPatient.is_claimed"
                  class="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl p-3 flex items-center justify-center gap-2 shadow-sm"
                >
                  <svg
                  ...
                  <span class="text-sm font-bold text-emerald-700 dark:text-emerald-400"
                    >Application Mobile Connectée</span
                  >
                </div>
                ...
                <div class="border-b border-gray-200 dark:border-zinc-800">
                  <nav class="-mb-px flex gap-4" aria-label="Tabs">
                    <button
                      v-if="!['secretary', 'assistant'].includes(role)"
                ...
                <div v-else class="space-y-3 pb-6">
                  ...
                  <div v-if="activeTab === 'dossier'" class="space-y-4">
                    <div
                      class="p-4 bg-rose-50/50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/30 rounded-xl shadow-sm"
                    >
                      <h4
                        class="text-xs font-bold text-rose-500 uppercase tracking-wider flex items-center gap-2 mb-2"
                      >
                      ...
                      <p
                        class="text-sm font-medium text-gray-900 dark:text-white whitespace-pre-line"
                      >
                    ...
                    <div
                      class="p-4 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl shadow-sm"
                    >
                      ...
                      <p
                        class="text-sm font-medium text-gray-900 dark:text-white whitespace-pre-line"
                      >
                        {{ selectedPatient.chronic_diseases || 'Non renseigné.' }}
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p
                        class="text-sm font-medium text-gray-900 dark:text-white whitespace-pre-line"
                      >
                        {{ selectedPatient.surgical_history || 'Non renseigné.' }}
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <p
                        class="text-sm font-medium text-gray-900 dark:text-white whitespace-pre-line"
                      >
                        {{ selectedPatient.family_history || 'Non renseigné.' }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div v-if="activeTab === 'appointments'" class="space-y-3">
                    <div
                      v-if="patientHistory.appointments.length === 0"
                      class="text-center py-6 text-gray-400 text-xs"
                    >
                      Aucun rendez-vous
                    </div>
                    <div
                      v-for="app in patientHistory.appointments"
                      :key="app.code"
                      class="p-3 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl shadow-sm flex items-center justify-between group hover:border-teal-200 transition-colors"
                    >
                      <div>
                      ...
                      <div class="flex flex-col items-end gap-1.5">
                        ...
                        <span
                          ...
                          class="text-[9px] text-gray-500 bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded font-bold flex justify-end w-full"
                          >Pas de consultation</span
                        >
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-if="activeTab === 'consultations'" class="space-y-3">
                    <div
                      v-if="patientHistory.consultations.length === 0"
                      class="text-center py-6 text-gray-400 text-xs"
                    >
                      Aucune consultation
                    </div>
                    <div
                      v-for="cons in patientHistory.consultations"
                      :key="cons.id"
                    ...
                    >
                      <div class="flex justify-between items-center mb-1">
                        ...
                        <svg
                          :class="[
                            ...
                            expandedConsultation === cons.id ? 'rotate-180' : '',
                          ]"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                      ...
                      <div
                        v-if="expandedConsultation !== cons.id"
                      ...
                      >
                        ...
                        <span
                          class="text-[10px] font-medium text-teal-600 bg-teal-50 dark:bg-teal-500/10 dark:text-teal-400 px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
                        >
                      ...
                      <div
                        v-if="expandedConsultation === cons.id"
                        class="mt-4 space-y-4 border-t border-gray-100 dark:border-zinc-800 pt-4"
                      >
                        ...
                        <div v-if="cons.diagnosis">
                          ...
                          >
                          <p
                            class="text-xs text-emerald-700 dark:text-emerald-400 mt-0.5 font-medium"
                          >
                            {{ cons.diagnosis }}
                          </p>
                        </div>
                        <div v-if="cons.prescription">
                          <span class="text-[10px] text-gray-400 uppercase font-bold"
                            >Prescription (Texte)</span
                          >
                          <p
                            class="text-xs text-gray-700 dark:text-gray-300 mt-0.5 whitespace-pre-wrap"
                          >
                            {{ cons.prescription }}
                          </p>
                        </div>
                        <div v-if="cons.notes">
                          <span class="text-[10px] text-gray-400 uppercase font-bold"
                            >Notes internes</span
                          >
                          <p class="text-xs text-gray-500 italic mt-0.5 whitespace-pre-wrap">
                            {{ cons.notes }}
                          </p>
                        </div>
                      </div>
                  ...
                  <div v-if="activeTab === 'ordonnances'" class="space-y-3">
                    <div
                      v-if="patientHistory.ordonnances.length === 0"
                      class="text-center py-6 text-gray-400 text-xs"
                    >
                      Aucune ordonnance
                    </div>
                    <div
                      v-for="ord in patientHistory.ordonnances"
                      :key="ord.code"
                      class="p-3 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl shadow-sm hover:border-purple-200 transition-colors flex items-center justify-between"
                    >
                      <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">
                          Ordonnance
                      ...
                      <a
                        v-if="ord.pdf_file"
                        :href="getPdfUrl(ord.pdf_file)"
                        target="_blank"
                        class="p-2 bg-purple-50 hover:bg-purple-100 dark:bg-purple-500/10 dark:hover:bg-purple-500/20 text-purple-600 rounded-lg transition-colors cursor-pointer"
  ...
  try {
    ...
    patients.value = rows.map((item) => ({
      ...
      lastVisitFormatted: item.last_consultation
        ...
        : 'Pas de visite',
      dateJoined: item.added_at ? new Date(item.added_at).toLocaleDateString('fr-FR') : '-',
      status: 'Stable',
