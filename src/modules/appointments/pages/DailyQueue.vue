    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500">Gestion interactive en temps réel de votre file d'attente</p>
        </div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Waiting Card -->
        <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex flex-col justify-center">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex flex-col justify-center">
          <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">En Consultation</p>
        ...
        <!-- Total Today Card -->
        <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex flex-col justify-center">
          <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Total Aujourd'hui</p>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-[#1A1C23] text-white rounded-3xl p-8 shadow-md relative overflow-hidden border border-gray-800">
          <div class="absolute -right-16 -top-16 w-64 h-64 rounded-full bg-blue-500/10 blur-3xl"></div>
          
          <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
            <div class="space-y-3">
              <span class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-blue-400 bg-blue-500/15 rounded-full uppercase tracking-wider">
                <span class="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></span>
                Patient Actuellement en Consultation
              </span>
              
              <div v-if="activePatient" class="space-y-1">
                <h2 class="text-3xl font-extrabold">{{ activePatient.patient_first_name }} {{ activePatient.patient_last_name }}</h2>
                <p class="text-sm text-gray-400 font-mono flex items-center gap-3">
                  <span>Code: <strong class="text-white">{{ activePatient.code }}</strong></span>
                  <span>•</span>
              ...
              <div v-else>
                ...
                <p class="text-sm text-gray-500 mt-1">Veuillez appeler le prochain patient de la file d'attente.</p>
              </div>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
          ...
          <div v-if="waitingList.length > 0" class="space-y-3">
            ...
            >
              <div class="flex items-center gap-5">
                <div class="w-12 h-12 rounded-xl bg-blue-50 text-blue-600 font-bold text-lg flex items-center justify-center shrink-0 shadow-sm">
                  #0{{ index + 1 }}
                </div>
                <div>
                  <h4 class="font-bold text-gray-900 text-lg">{{ item.patient_first_name }} {{ item.patient_last_name }}</h4>
                  <p class="text-xs text-gray-400 mt-1 font-mono flex items-center gap-3">
                    <span>Code: <strong class="text-gray-500">{{ item.code }}</strong></span>
                    <span>•</span>
                    <span>Tél: {{ item.patient_phone }}</span>
                    <span>•</span>
                    <span class="px-2 py-0.5 bg-gray-200/60 rounded-md font-bold text-[10px] uppercase text-gray-500">
              ...
              <div class="flex items-center gap-3 self-end sm:self-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          ...
          <div v-else class="text-center py-16 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
            ...
            <p class="text-sm text-gray-400 mt-1">Tous les rendez-vous d'aujourd'hui ont été pris en charge.</p>
          </div>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <div class="flex justify-between items-center border-b border-white/10 pb-6">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center font-bold text-lg">P</div>
            <div>
          ...
          <div class="flex items-center gap-6">
            ...
            <button @click="tvModeOpen = false" class="text-xs text-gray-500 hover:text-white px-3 py-1 bg-white/5 rounded-lg border border-white/10">
              Quitter le mode TV
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 grow py-12 items-center">
          ...
          <div class="space-y-6 text-center lg:text-left bg-blue-900/10 border border-blue-500/20 p-10 rounded-[32px] shadow-2xl relative overflow-hidden">
            <div class="absolute -right-24 -bottom-24 w-80 h-80 rounded-full bg-blue-600/10 blur-3xl"></div>
            <span class="px-4 py-1.5 bg-blue-500 text-white rounded-full text-xs font-bold uppercase tracking-widest">
              PATIENT APPELÉ
            </span>
            
            <div v-if="activePatient" class="space-y-4">
              <h1 class="text-6xl font-extrabold tracking-tight text-white mt-4">
                {{ activePatient.patient_first_name }} {{ getInitials(activePatient.patient_last_name) }}.
              </h1>
              <p class="text-8xl font-black text-blue-400 tracking-wider">
                {{ activePatient.code }}
              </p>
              <p class="text-lg text-gray-400 mt-2">Veuillez vous présenter au cabinet de consultation du médecin.</p>
            </div>
            
            <div v-else class="py-12">
              <h3 class="text-3xl font-bold text-gray-500">Aucun appel actif</h3>
              <p class="text-sm text-gray-400 mt-2">Veuillez patienter, le médecin va appeler le prochain patient sous peu.</p>
            </div>
          </div>

          <!-- Right side: Next tickets in the queue -->
          <div class="space-y-6">
            <h3 class="text-lg font-bold tracking-widest text-gray-400 uppercase">PROCHAINS TICKET EN ATTENTE</h3>
            ...
            <div class="space-y-4 max-h-[480px] overflow-hidden">
              ...
              >
                <div class="flex items-center gap-6">
                  <span class="w-12 h-12 rounded-xl bg-white/5 font-extrabold text-lg flex items-center justify-center">
                    #0{{ index + 1 }}
                  </span>
                  <div>
              ...
              <!-- Fallback -->
              <p v-if="waitingList.length === 0" class="text-gray-500 text-lg py-12">File d'attente vide.</p>
            </div>
          </div>
        </div>

        <!-- TV footer status bar -->
        <div class="border-t border-white/5 pt-6 text-center text-xs text-gray-500 flex justify-between items-center">
      ...
      <div v-if="ticketPrintData" class="print-ticket hidden print:block w-[80mm] p-6 text-black bg-white font-mono text-center mx-auto">
        ...
        <!-- Embedded QR Code -->
        <div class="w-28 h-28 border-4 border-black mx-auto my-3 flex items-center justify-center p-1 bg-white">
        ...
        <div class="text-left text-[11px] space-y-1 mt-4 border-t border-dashed border-gray-400 pt-3">
          <p><strong>Patient:</strong> {{ ticketPrintData.patient_first_name }} {{ ticketPrintData.patient_last_name }}</p>
...
const fetchQueueData = async () => {
  try {
    ...
    if (waitingList.value.length === 0 && !activePatient.value) {
      ...
      waitingList.value = [
        { code: 'AP-T456', patient_first_name: 'Ahmed', patient_last_name: 'Benali', patient_phone: '+213 666 45 89 12', period: 'morning' },
        { code: 'AP-R123', patient_first_name: 'Fatiha', patient_last_name: 'Bouhired', patient_phone: '+213 777 98 45 22', period: 'afternoon' }
      ]
...
const updateTime = () => {
  const now = new Date()
  liveTime.value = now.toLocaleTimeString('fr-DZ', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
}
