<template>
  <div class="space-y-8 p-1">
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
      <!-- Total Card -->
      <div
        class="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800/60 dark:bg-zinc-900/40 backdrop-blur-sm"
      >
        <div class="flex items-center justify-between">
          <p class="text-xs font-medium tracking-wider text-gray-400 uppercase">Total</p>
          <div
            class="h-8 w-8 rounded-lg bg-gray-50 dark:bg-zinc-800 flex items-center justify-center text-gray-500"
          >
            <svg
              class="w-4 h-4"
              fill="none"
        ...
        <p class="text-3xl font-semibold tracking-tight text-gray-900 dark:text-white mt-2">
          {{ appointments.length }}
        </p>
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="h-8 w-8 rounded-lg bg-teal-50/60 dark:bg-teal-500/10 flex items-center justify-center text-teal-600 dark:text-teal-400"
          >
            <svg
              class="w-4 h-4"
              fill="none"
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="h-8 w-8 rounded-lg bg-emerald-50/60 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600 dark:text-emerald-400"
          >
            <svg
              class="w-4 h-4"
              fill="none"
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="h-8 w-8 rounded-lg bg-rose-50/60 dark:bg-rose-500/10 flex items-center justify-center text-rose-600 dark:text-rose-400"
          >
            <svg
              class="w-4 h-4"
              fill="none"
    ...
    <!-- Filters Toolbar -->
    <div
      class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-gray-50/50 dark:bg-zinc-900/20 p-2 rounded-xl"
    >
        ...
        <table class="min-w-full table-fixed">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800/60">
            ...
            >
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="truncate">
                    <span
                      ...
                      >{{ app.patientName || 'Patient inconnu' }}</span
                    >
                ...
                <div class="mt-1">
                  ...
                  >
                    <span
                      ...
                      :class="app.period === 'morning' ? 'bg-amber-500' : 'bg-indigo-500'"
                    ></span>
              ...
              <td class="px-6 py-3.5 whitespace-nowrap">
                <span
                  class="text-xs font-mono text-gray-500 dark:text-zinc-400 bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 px-2 py-0.5 rounded-md"
                >
                  {{ app.code }}
                </span>
              </td>
        ...
        >
          ...
          <div class="p-5 space-y-6 flex-1 overflow-y-auto">
            ...
            >
              ...
              >
                <svg
                  v-if="isMale(selectedAppointment.gender)"
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-7 h-7"
                  fill="none"
                  viewBox="0 0 24 24"
              ...
              <h3 class="text-base font-medium text-gray-900 dark:text-white">
                {{ selectedAppointment.patientName || 'Patient inconnu' }}
              </h3>
              <p class="text-xs text-gray-400 font-mono mt-0.5">
                {{ selectedAppointment.patientPhone || '—' }}
              </p>
              <p v-if="selectedAppointment.email" class="text-xs text-gray-400 font-mono mt-0.5">
                {{ selectedAppointment.email }}
              </p>

              <div class="flex gap-2 mt-3 flex-wrap justify-center">
                <span
                  v-if="selectedAppointment.bloodGroup"
                  class="text-[10px] px-2 py-0.5 bg-red-50 text-red-600 border border-red-100 rounded-md font-medium"
                  >Sang: {{ selectedAppointment.bloodGroup }}</span
                >
                <span
                  v-if="selectedAppointment.dateOfBirth"
                  class="text-[10px] px-2 py-0.5 bg-teal-50 text-teal-600 border border-teal-100 rounded-md font-medium"
                  >Né(e) le: {{ selectedAppointment.dateOfBirth }}</span
                >
              </div>
            </div>
            ...
            <div class="space-y-2.5">
              <div
                class="flex items-center justify-between p-3 rounded-lg border border-gray-100 dark:border-zinc-900"
              >
              ...
              >
                ...
                <span class="text-xs font-medium text-gray-800 dark:text-zinc-200">{{
                  selectedAppointment.period === 'morning'
                    ? 'Matin (08:00 - 12:00)'
...
const fetchAppointments = async () => {
  try {
    ...
    appointments.value = data.map((item) => {
      const patientGender =
        item.gender || item.patient?.gender || item.patient_details?.gender || 'M'
...
const filteredAppointments = computed(() => {
  return appointments.value.filter((app) => {
    ...
    return Object.values(app).some((value) => String(value).toLowerCase().includes(query))
  })
})
