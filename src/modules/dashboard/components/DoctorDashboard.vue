<template>
  <div class="space-y-8 text-gray-900 font-sans w-full">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div
        class="bg-gradient-to-br from-brand-600 to-brand-800 text-white rounded-[24px] p-8 shadow-xl shadow-brand-900/10 relative overflow-hidden flex flex-col justify-between min-h-[280px]"
      >
        <div class="absolute -right-10 -top-10 w-48 h-48 rounded-full bg-white/10 blur-3xl"></div>
        <div
          class="absolute -left-10 -bottom-10 w-40 h-40 rounded-full bg-brand-400/20 blur-2xl"
        ></div>
        ...
        <div class="relative z-10">
          ...
          <div v-if="queue.current" class="space-y-4">
            <div>
              ...
              <p class="text-sm text-brand-200 font-mono mt-1 opacity-80">
                Dossier: {{ queue.current.code }}
              </p>
            </div>
          ...
          <div
            v-else
            class="py-8 flex flex-col items-center justify-center text-center opacity-80 mt-4"
          >
      ...
      <!-- Next Patient Card -->
      <div
        class="bg-white rounded-[24px] border border-gray-100 p-8 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col justify-between min-h-[280px]"
      >
        <div class="flex items-center justify-between mb-6">
          <span class="text-sm font-bold text-gray-400 uppercase tracking-widest"
            >Prochain Patient</span
          >
          <span
            class="text-xs font-bold text-brand-700 bg-brand-50 px-3 py-1.5 rounded-full ring-1 ring-brand-100"
          >
        ...
        <div
          v-if="queue.waiting && queue.waiting.length > 0"
          class="flex-1 flex flex-col justify-between"
        >
          <div class="flex items-center gap-5">
            <div
              class="w-16 h-16 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center text-2xl font-bold ring-4 ring-white shadow-sm"
            >
              {{ queue.waiting[0].patient_name.charAt(0).toUpperCase() }}
            </div>
            <div>
              <h2 class="text-2xl font-bold text-gray-900 tracking-tight">
                {{ queue.waiting[0].patient_name }}
              </h2>
              <div class="flex items-center gap-3 mt-1.5">
                <span class="text-sm text-gray-500 font-mono">{{ queue.waiting[0].code }}</span>
                <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{
                  periodLabel(queue.waiting[0].period)
                }}</span>
              </div>
            </div>
          </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
      ...
      >
        ...
        <h3 class="text-3xl font-black text-gray-900">
          {{ dashboardStats?.today_total || todayPatients.length }}
        </h3>
      </div>
      <div
        class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow"
      >
        ...
        <h3 class="text-3xl font-black text-brand-600">
          {{ dashboardStats?.today_waiting || queue.waiting_count }}
        </h3>
      </div>
      <div
        class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow"
      >
        ...
        <h3 class="text-3xl font-black text-emerald-600">
          {{
            dashboardStats?.today_completed ||
            todayPatients.length - queue.waiting_count - (queue.current ? 1 : 0)
          }}
        </h3>
      </div>
      ...
      <router-link
        to="/queue/today"
        class="bg-gray-900 hover:bg-gray-800 text-white rounded-2xl p-6 flex flex-col justify-center items-center text-center transition-all group shadow-lg shadow-gray-900/10"
      >
        <div
          class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform"
        >
    ...
    <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
      <div class="flex items-center justify-between mb-8">
        ...
        <router-link
          to="/patients"
          class="text-sm font-bold text-brand-600 hover:text-brand-700 bg-brand-50 px-4 py-2 rounded-xl transition-colors"
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr
              class="border-b border-gray-100 text-gray-400 text-xs font-bold uppercase tracking-widest"
            >
          ...
          <tbody class="divide-y divide-gray-50 text-sm">
            <tr
              v-for="(pat, idx) in todayPatients.slice(0, 5)"
              :key="pat.code"
              class="group hover:bg-gray-50 transition-colors"
            >
              <td class="py-5 px-4">
                <span
                  class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-gray-500 font-mono font-bold text-xs"
                >
              ...
              <td class="py-5 text-right px-4">
                <router-link
                  :to="`/consultations/create?patient=${pat.code}`"
                  class="inline-flex items-center gap-1 text-brand-600 hover:text-brand-800 font-bold text-sm transition-colors group-hover:underline"
                >
                  Consulter
            ...
            <tr v-if="loadingTodayPatients">
              <td colspan="4" class="py-12 text-center">
                <div class="flex flex-col items-center justify-center space-y-3">
                  <div
                    class="w-8 h-8 border-4 border-gray-200 border-t-brand-600 rounded-full animate-spin"
                  ></div>
