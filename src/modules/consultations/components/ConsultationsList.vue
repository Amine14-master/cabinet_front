  ...
  <div class="space-y-4">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
      ...
      <div class="flex items-center gap-2">
        <div class="px-3 py-1.5 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-lg text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800/80 dark:bg-zinc-950/40 backdrop-blur-sm">
        ...
        <table class="min-w-full table-fixed">
          <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800/80 bg-gray-50/40 dark:bg-zinc-900/10">
              ...
              <th class="w-1/4 px-6 py-3.5 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">Date RDV</th>
              <th class="w-1/4 px-6 py-3.5 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">Diagnostic</th>
              <th class="w-1/4 px-6 py-3.5 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">Prescription</th>
            ...
            <tr v-else-if="paginatedData.length === 0">
              <td colspan="5" class="px-6 py-16 text-center">
                <div class="flex flex-col items-center justify-center max-w-xs mx-auto">
                  ...
                  <p class="text-sm font-medium text-gray-700 dark:text-zinc-300">Aucune consultation trouvée</p>
            ...
            >
              <td class="px-6 py-3.5 whitespace-nowrap">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-full flex items-center justify-center shadow-inner transition-transform group-hover:scale-[1.02] bg-blue-100 text-blue-600 dark:bg-blue-500/20">
                    <span class="text-sm font-semibold">{{ item.patientName ? item.patientName.charAt(0).toUpperCase() : 'P' }}</span>
                  </div>
                  <div class="truncate">
                    <span class="block text-sm font-medium text-gray-800 dark:text-zinc-200 truncate">{{ item.patientName }}</span>
                    <span class="block text-xs text-gray-400 font-mono tracking-tight mt-0.5">RDV: {{ item.appointmentCode }}</span>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-3.5 max-w-[220px] truncate">
                <p class="text-sm text-gray-800 dark:text-gray-200 truncate" :title="item.diagnosis">
                  {{ item.diagnosis || '---' }}
                </p>
              </td>

              <td class="px-6 py-3.5 max-w-[220px] truncate">
                <p class="text-xs text-gray-500 dark:text-gray-400 italic truncate" :title="item.prescription">
                  {{ item.prescription || '---' }}
                </p>
              </td>
      ...
      <div v-if="filteredData.length > 0" class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800/80 bg-gray-50/20 dark:bg-zinc-900/5">
        <div class="text-xs text-gray-400">
          Affichage de <span class="font-medium text-gray-700 dark:text-zinc-300">{{ startIndex + 1 }}</span> à 
          <span class="font-medium text-gray-700 dark:text-zinc-300">{{ Math.min(endIndex, filteredData.length) }}</span> sur 
          <span class="font-medium text-gray-700 dark:text-zinc-300">{{ filteredData.length }}</span> résultats
        </div>
        <div class="flex items-center gap-2">
          <button 
            @click="prevPage" 
    ...
    <Transition name="slide">
      <div v-if="selectedItem" class="fixed inset-0 z-50 overflow-hidden">
        ...
        <div class="absolute inset-y-0 right-0 max-w-md w-full bg-white dark:bg-zinc-950 border-l border-gray-100 dark:border-zinc-900 shadow-xl flex flex-col">
          <div class="p-5 border-b border-gray-100 dark:border-zinc-900 flex justify-between items-center">
            <div>
              <h2 class="text-base font-semibold text-gray-900 dark:text-white">Fiche de Consultation</h2>
              <p class="text-xs font-mono text-gray-400 mt-0.5">Code RDV: {{ selectedItem.appointmentCode }}</p>
            </div>
          ...
          <div class="p-5 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
            <div class="flex flex-col items-center text-center p-4 rounded-xl bg-gray-50/50 dark:bg-zinc-900/30 border border-gray-100/50 dark:border-zinc-900">
              <div class="w-14 h-14 rounded-xl bg-blue-600 text-white flex items-center justify-center text-lg font-bold shadow-md shadow-blue-500/10 mb-3">
                {{ selectedItem.patientName ? selectedItem.patientName.charAt(0).toUpperCase() : 'P' }}
              </div>
              <h3 class="text-base font-medium text-gray-900 dark:text-white">{{ selectedItem.patientName }}</h3>
              <p class="text-xs text-gray-400 font-mono mt-0.5">{{ selectedItem.patientPhone || 'Pas de téléphone' }}</p>
            </div>

            <div class="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-zinc-900 bg-gray-50/20">
              <span class="text-xs text-gray-400 font-medium">Date d'enregistrement</span>
              <span class="text-xs font-semibold text-gray-800 dark:text-zinc-200">{{ selectedItem.date }}</span>
            </div>

            <div class="space-y-1.5">
              <span class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Symptômes signalés</span>
              <div class="text-xs text-gray-800 dark:text-zinc-300 bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800/60 p-3.5 rounded-xl leading-relaxed whitespace-pre-line min-h-[70px]">
                {{ selectedItem.symptoms || 'Aucun symptôme mentionné.' }}
              </div>
            </div>

            <div class="space-y-1.5">
              <span class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Diagnostic Médical</span>
              <div class="text-xs font-medium text-emerald-700 dark:text-emerald-400 bg-emerald-50/40 dark:bg-emerald-500/[0.02] border border-emerald-100 dark:border-emerald-800/30 p-3.5 rounded-xl whitespace-pre-line">
                {{ selectedItem.diagnosis || 'Aucun diagnostic posé.' }}
              </div>
            </div>

            <div class="space-y-1.5">
              <span class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Ordonnance & Traitement</span>
              <div class="text-xs font-mono text-gray-800 dark:text-zinc-300 bg-blue-50/20 dark:bg-blue-500/[0.01] border border-blue-100/50 dark:border-zinc-800/40 p-4 rounded-xl leading-relaxed whitespace-pre-line">
                <div class="border-b border-blue-100/50 dark:border-blue-900/30 pb-1.5 mb-2.5 text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider text-[10px]">
                  💊 Médicaments
                </div>
          ...
          <div class="p-5 border-t border-gray-100 dark:border-zinc-900 bg-white dark:bg-zinc-950">
            <button @click="closeDetails" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-xl text-sm transition-all active:scale-[0.98]">
              Fermer le dossier
            </button>
          </div>
        </div>
