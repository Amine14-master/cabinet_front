...
export const updateStaff = async (code: string, data: StaffCreateUpdate) => {
  const response = await api.put(`doctors/staff/${code}/`, data)

  return response.data
}

export const deleteStaff = async (code: string) => {
  const response = await api.delete(`doctors/staff/${code}/deactivate/`)

  return response.data
}

export const getStaffPermissions = async (code: string) => {
  const response = await api.get(`doctors/staff/${code}/permissions/`)

  return response.data
}

export const updateStaffPermissions = async (code: string, data: StaffPermission) => {
  const response = await api.put(`doctors/staff/${code}/permissions/`, data)
