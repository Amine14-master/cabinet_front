...
export const getAppointmentForConsultation = async (appointmentCode: string): Promise<AppointmentForConsultation> => {
...
export const searchMedicaments = async (
  query: string
): Promise<MedicamentSearchResult[]> => {
...
export const getDoctorMedicaments = async (): Promise<DoctorMedicament[]> => {
  const { data } = await api.get(
    "appointments/medicaments/doctor/"
  );
...
export const deactivateMedicament = async (id: number) => {
  await api.delete(
    `appointments/medicaments/${id}/deactivate/`
  )
}