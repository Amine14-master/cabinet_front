          ...
          <div class="lg:col-span-4 flex flex-col space-y-6">
            ...
            <div class="bg-white dark:bg-zinc-950 rounded-3xl border border-slate-200 dark:border-zinc-800 p-6 shadow-sm">
              <div class="flex items-center gap-3 mb-6">
                <div class="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-brand-600">
              ...
              <div v-if="!selectedPatient" class="space-y-4">
                ...
                <div v-if="searching" class="text-center py-8">
                  <div class="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-brand-600 border-r-transparent"></div>
                  <p class="text-sm text-gray-500 mt-2">Recherche...</p>
                </div>

                <!-- Search Results -->
                <div v-if="!searching && searchResults.length > 0" class="border border-slate-100 dark:border-zinc-800 rounded-2xl overflow-hidden divide-y divide-slate-50 dark:divide-zinc-800">
                  <div 
                  ...
                  >
                    <div class="flex-1">
                      <div class="flex items-center gap-2">
                        <span class="text-xs font-mono bg-slate-100 dark:bg-zinc-800 px-2 py-1 rounded-lg text-slate-600 dark:text-slate-400 font-bold">
                          {{ patient.code }}
                        </span>
                ...
                <div v-if="!searching && searchQuery.length >= 2 && searchResults.length === 0" 
                  ...
                  <div class="flex items-center gap-2 mb-4">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand-100 dark:bg-brand-900 text-xs text-brand-600 font-bold">+</span>
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Nouveau Patient</p>
                  </div>
                  <div class="space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                      <input v-model="form.patient_first_name" type="text" placeholder="Prénom *" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-700 dark:bg-zinc-900 focus:ring-2 focus:ring-brand-500 outline-none shadow-sm text-sm" />
                    ...
                    <input v-model="form.patient_phone" type="tel" placeholder="Numéro de téléphone *" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-700 dark:bg-zinc-900 focus:ring-2 focus:ring-brand-500 outline-none shadow-sm text-sm" />
                  </div>
                </div>
              </div>
              ...
              <div v-else class="relative overflow-hidden">
                <div class="bg-brand-50 border border-brand-100 dark:border-brand-900/30 dark:bg-brand-900/10 rounded-2xl p-5 shadow-sm">
                  <div class="flex items-start justify-between">
                    <div>
                      <p class="text-[10px] font-bold text-brand-600 uppercase tracking-wider mb-2">Patient sélectionné</p>
                      ...
                      <h3 class="text-lg font-bold text-gray-900 dark:text-white leading-tight mb-2">{{ selectedPatient.first_name }} {{ selectedPatient.last_name }}</h3>
                      <div class="space-y-1">
                        <p v-if="selectedPatient.phone" class="text-gray-500 dark:text-gray-400 text-xs flex items-center gap-1.5">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                          {{ selectedPatient.phone }}
                        </p>
                      </div>
                    </div>
                    <button @click="clearSelectedPatient" type="button" class="h-8 w-8 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 hover:bg-slate-50 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 transition-all shadow-sm">
            ...
            <div class="bg-white dark:bg-zinc-950 rounded-3xl border border-slate-200 dark:border-zinc-800 p-6 shadow-sm">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <label class="cursor-pointer relative">
                  ...
                  <div class="p-4 rounded-2xl border-2 border-slate-100 dark:border-zinc-800 hover:border-brand-200 peer-checked:border-brand-600 peer-checked:bg-brand-50 dark:peer-checked:bg-brand-900/20 transition-all text-center">
                    <span class="block text-2xl mb-1">??</span>
                    <span class="block text-sm font-bold text-gray-900 dark:text-white">Après-midi</span>
          ...
          <div class="lg:col-span-8 flex flex-col">
            <div class="bg-white dark:bg-zinc-950 rounded-3xl border border-slate-200 dark:border-zinc-800 p-6 xl:p-8 shadow-sm flex-1 flex flex-col relative overflow-hidden">
              ...
              <transition name="fade">
                <div v-if="errorMessage" class="mt-6">
                  <div class="p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100 dark:border-red-800">
                    <div class="flex items-center gap-3">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              ...
              <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 mt-8 pt-6 border-t border-slate-100 dark:border-zinc-800">
                ...
                <button type="submit" :disabled="loading || !isFormValid" class="w-full sm:w-auto px-8 py-2.5 rounded-xl bg-brand-600 text-white font-bold hover:bg-brand-700 shadow-md shadow-brand-500/20 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm">
...
const monthName = computed(() => {
  const name = new Date(currentYear.value, currentMonth.value).toLocaleString('fr-FR', { month: 'long', year: 'numeric' })
